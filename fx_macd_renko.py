...
con = fxcmpy.fxcmpy(config_file= "fxcm.cfg", log_level = 'error', server='demo')
#defining strategy parameters
pairs = ['LTC/USD','ETH/USD'] #currency pairs to be included in the strategy
...
def main():
    try:
        ...
        for currency in pairs:
            ...
            if signal == "Buy":
                con.open_trade(symbol=currency, is_buy=True, is_in_pips=True, amount=pos_size[currency], 
                               time_in_force='GTC', stop=-8, trailing_step =True, order_type='AtMarket')
                print("New long position initiated for ", currency)
            elif signal == "Sell":
                con.open_trade(symbol=currency, is_buy=False, is_in_pips=True, amount=pos_size[currency], 
                               time_in_force='GTC', stop=-8, trailing_step =True, order_type='AtMarket')
